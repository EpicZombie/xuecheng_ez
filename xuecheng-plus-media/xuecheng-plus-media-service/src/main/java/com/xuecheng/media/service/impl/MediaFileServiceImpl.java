...
public class MediaFileServiceImpl implements MediaFileService {
 ...
 String bucket_videoFiles = "testbucket";
 ...
 public UploadFileResultDto uploadFile(Long companyId, UploadFileParamsDto uploadFileParamsDto, String localFilePath) {
  ...
  if (!file.exists()) {
   XueChengEzException.cast("文件不存在");
  }
  //文件名称
  ...
  if(!b){
    XueChengEzException.cast("上传文件失败");
  }
  //文件大小
  ...
  if(mediaFiles == null){
    XueChengEzException.cast("文件上传后保存信息失败");
  }
  //准备返回数据
...
//   log.debug("合并文件失败,fileMd5:{},异常:{}",fileMd5,e.getMessage(),e);
   return RestResponse.validfail(false,"失败");
  }
...
//   log.debug("校验文件失败,fileMd5:{},异常:{}",fileMd5,e.getMessage(),e);
  ...
  if(mediaFiles == null){
    return RestResponse.validfail(false,"文件写入DB失败");
  }
...
//     log.error("清楚分块文件失败,objectname:{}",deleteError.objectName(),e);
     System.out.printf("清除分块文件失败{}\n", deleteError.objectName());
    }
   });
  } catch (Exception e) {
   e.printStackTrace();
//   log.error("清楚分块文件失败,chunkFileFolderPath:{}",chunkFileFolderPath,e);
   System.out.println("清除分块文件失败");
  }
 }
...
//    log.error("保存文件信息到数据库失败,{}",mediaFiles.toString());
    XueChengEzException.cast("保存文件信息失败");
   }
...
//   log.error("上传文件到minio出错,bucket:{},objectName:{},错误原因:{}",bucket,objectName,e.getMessage(),e);
   XueChengEzException.cast("上传文件到文件系统失败");
  }
  return false;
 }
